...
import authorsPageInitialPropsTqPFResource from '../../resources/authors-page-initial-props-tq_p-f'
import authorsPageInitialPathsTqI3Resource from '../../resources/authors-page-initial-paths-tq_i3'
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialPropsTqPFResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsTqI3Resource({
      content_type: 'author',
      select: 'sys.id',
    })
    return {
