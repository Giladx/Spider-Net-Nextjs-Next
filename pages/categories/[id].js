...
import categoriesPageInitialPropsTqBSResource from '../../resources/categories-page-initial-props-tq_b-s'
import categoriesPageInitialPathsTqUxResource from '../../resources/categories-page-initial-paths-tq_ux'
...
export async function getStaticProps(context) {
  try {
    const response = await categoriesPageInitialPropsTqBSResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await categoriesPageInitialPathsTqUxResource({
