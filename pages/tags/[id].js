...
import tagsPageInitialPropsTqS0Resource from '../../resources/tags-page-initial-props-tq_s0'
import tagsPageInitialPathsTq0aResource from '../../resources/tags-page-initial-paths-tq_0a'
...
export async function getStaticProps(context) {
  try {
    const response = await tagsPageInitialPropsTqS0Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await tagsPageInitialPathsTq0aResource({
      content_type: 'tag',
      select: 'sys.id',
    })
    return {
